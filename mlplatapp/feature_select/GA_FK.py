...
class GA(object):
    ...
    def cal_all_fitness(self):
        ...
        for i in range(self.population_size):
            if np.any(np.array(self.population[i]) != 0):  # 避免变异可能导致的特征子集为空的状况
                if len(self.feature_kernel) == 0:
                    selected_feature_list = np.array(
                        [index for index, values in enumerate(self.population[i]) if values == 1])
                    options = selected_feature_list
                else:
                    selected_feature_list = self.not_kernel[
                        [index for index, values in enumerate(self.population[i]) if values == 1]]
                    options = np.append(self.feature_kernel, selected_feature_list)
    ...
    def crossover(self, parents):
        ...
        f_avg = np.mean(
            [self.fitness[index] for index in [population.index(parents[i].tolist()) for i in range(len(parents))]])
        f_max = np.max(
            [self.fitness[index] for index in [population.index(parents[i].tolist()) for i in range(len(parents))]])
        children = []
    ...
    def evolve(self):
        ...
        for i in range(len(children)):
            if len(self.feature_kernel) == 0:
                ...
                selected_feature_list = self.not_kernel[
                    [index for index, values in enumerate(children[i]) if values == 1]]
                options = np.append(self.feature_kernel, selected_feature_list)
            data_x = self.data.X[:, options]
            data_y = self.data.Y
            avg_rmse, r2 = self.cal_fitness(data_x, data_y)
